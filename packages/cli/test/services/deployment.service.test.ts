...
describe('DeploymentService', () => {
	...
	it('should execute pulumi preview for dry run', async () => {
		...
		await deploymentService.execute(mockCicdEngine, cicdConfig, 'my-app', {
			dryRun: true,
			environment: 'dev'
		});
	...
	it('should handle pulumi stack init idempotency', async () => {
		...
		await expect(
			deploymentService.execute(mockCicdEngine, cicdConfig, 'my-app', {
				dryRun: false,
				environment: 'dev'
			})
	...
	it('should clean up temporary directory even on pulumi failure', async () => {
		...
		).rejects.toThrowError('Pulumi up failed');
	...
	it('should throw an error if a required secret is missing', async () => {
		...
		).rejects.toThrowError('Secret DOCKER_USERNAME not found');
	});
	...
	it('should deploy a deployable without secrets if none are required', async () => {
		...
		await deploymentService.execute(mockCicdEngine, cicdConfig, 'another-app', {
			dryRun: false,
			environment: 'dev'
		});
	...
	it('should perform topological sort and deploy all deployables in correct order', async () => {
		...
		vi.spyOn(mockCicdEngine.plugins.noSecretsPlugin, 'deployHandler').mockImplementation(
			async (config) => {
				deployOrder.push(config.name);
			}
		);

		await deploymentService.execute(mockCicdEngine, cicdConfig, undefined, {
			dryRun: false,
			environment: 'dev'
		});
	...
	it('should throw an error for circular dependencies', async () => {
		...
		await expect(
			deploymentService.execute(mockCicdEngine, cicdConfig, undefined, {
				dryRun: false,
				environment: 'dev'
			})
		).rejects.toThrowError('Circular dependency detected or some deployables are not reachable.');
	});
});
