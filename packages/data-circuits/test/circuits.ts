...
export type Subscriber<T, TAcceptsUndefined extends boolean = true> = TAcceptsUndefined extends true
...
export const createCircuit = <TOutput, TInitial extends TOutput | undefined>({
	name,
	getter,
	initialValue,
	store = createMemoryStore<TOutput>(),
	bad = 0,
	stale,
	staleOnError = false,
	...
	const ingestValueUpdate = async (data: TOutput) => {
		...
		lastValue = typeof data === 'object' ? JSON.parse(JSON.stringify(data)) : data;
		value = {
			data,
			bad: Date.now() + bad,
			stale: Date.now() + (stale ?? bad)
	...
	const get = async (refresh?: boolean): Promise<TOutput> => {
		console.log('get called', { refresh, value, stale: value?.stale, now: Date.now() });
		if (refresh || !value || Date.now() > value.stale) {
			console.log('getting from store');
			value = bad <= 0 ? undefined : await store.get(name).catch(() => undefined);
		}
		if (!refresh && value && Date.now() < value.stale) {
		...
		if (!refresh && value && Date.now() > value.bad) {
			console.log('value is bad, clearing');
			value = undefined;
		}
...
>({
	...
	logger,
	equalityChecker
}: DerivedCircuitOptions<TOutput, TSources, TInitial>): Circuit<TOutput, TInitial> => {
