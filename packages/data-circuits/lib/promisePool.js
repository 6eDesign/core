...
export const getDistributedPoolFactory =
	(redisClient) =>
	/**
	 * @template {import('./promisePools.memory').PoolPromise} T
	 * @param {T} prom
	 * @param {import('./promisePools.memory').PoolOptions} options
	 */
	(prom, { id, timeout = 5000 }) => {
		...
		redisClient.xgroup('CREATE', streamId, `${streamId}_g`, '$', 'MKSTREAM').catch((e) => {
		...
		const getLock = () => redlock.acquire([id], timeout).catch((e) => undefined);
