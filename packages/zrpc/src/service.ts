...
export interface ZRPCServiceOptions<TContext = Record<string, any>> {
	port?: number;
	name: string;
	useCors?: boolean;
...
export class ZRPCService<TContext = Record<string, any>> {
	...
	private resolverMap: Record<
		string,
		{ route: RouteOptions<any, any>; resolver: (input: any, context: any) => Promise<any> }
	> = {};
	private server?: http.Server;
	private readonly context?: (req: express.Request) => Promise<TContext>;

	constructor(options: ZRPCServiceOptions<TContext>) {
	...
	public addRoute<TInput, TOutput>(
		...
		this.resolverMap[getRouteKey(route)] = { route, resolver: resolver as any };

		const httpMethod = route.method ?? 'get';
		this.app[httpMethod](
			route.path,
			createHTTPResolver({ route, resolver, context: this.context })
		);
	}
	...
	private async handleMessage({ key = '', input }: { key?: string; input?: unknown } = {}) {
		...
		try {
			...
			await resolver(parsedInput, {}); // Empty context for now
		} catch (error) {
